...
class TestComplexOptimalInvariants:
    ...
    def test_images(self):
        ...
        x = repeat(test_img, f'c h w -> 1 c h w').to(dtype=get_default_complex())
        rot_input = repeat(rotated_img, f'c h w -> 1 c h w').to(dtype=get_default_complex())
        return x, rot_input
    ...
    def _test_90_module(module, test_images, test_device):
        ...
        for batch_idx in range(c_channels.shape[0]):
            for channel in range(c_channels.shape[1]):
                ...
                if gap < 1e-5:
                    zero_channels += 1
        assert zero_channels != 0, "Some channels coming from network are zero, which is not expected."
