...
class ResHNeXtv3(nn.Module):
    def __init__(self,
        ...
        self.classifier = GAPMLP(in_channels=_last_out_channels * (self.maximum_order + 1),
...
class RIC(nn.Module):
    def __init__(self, batch_size, input_size=32, num_classes=10, in_channels=1):
        super(RIC, self).__init__()              
        self.input_height = input_size
        self.input_width = input_size
        self.num_classes = num_classes
        # conv11
        self.coords11 = self.generate_coordinates(batch_size, self.input_height, self.input_width)
        self.conv11 = nn.Conv2d(in_channels,32,3,padding=1,stride=1)
        self.bn11 = nn.BatchNorm2d(32)
        # conv12
        self.coords12 = self.generate_coordinates(batch_size, self.input_height, self.input_width)
        self.conv12 = nn.Conv2d(32,32,3,padding=1,stride=1)
        self.bn12 = nn.BatchNorm2d(32)
        self.mp12 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
            
        # conv21
        self.coords21 = self.generate_coordinates(batch_size, self.input_height // 2, self.input_width // 2)
        self.conv21 = nn.Conv2d(32,64,3,padding=1,stride=1)
        self.bn21 = nn.BatchNorm2d(64)
        # conv22
        self.coords22 = self.generate_coordinates(batch_size, self.input_height // 2, self.input_width // 2)
        self.conv22 = nn.Conv2d(64,64,3,padding=1,stride=1)
        self.bn22 = nn.BatchNorm2d(64)
        self.mp22 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
                    
        # conv31
        self.coords31 = self.generate_coordinates(batch_size, self.input_height // 4, self.input_width // 4)
        self.conv31 = nn.Conv2d(64, 128, 3, padding=1,stride=1)
        self.bn31 = nn.BatchNorm2d(128)
        # conv32
        self.coords32 = self.generate_coordinates(batch_size, self.input_height // 4, self.input_width // 4)
        self.conv32 = nn.Conv2d(128, 128, 3, padding=1,stride=1)
        self.bn32 = nn.BatchNorm2d(128)
        self.ap32 = nn.AvgPool2d(kernel_size=self.input_height // 4, stride=self.input_width // 4, padding=0)
